...
describe('ESERCIZIO 45', () => {
    ...
    test('TEST 04', () => {
        expect(only_upper(
            fs.readFileSync('esercizio_js_45/input_test04.txt', 'utf-8')
        ).toString()).toBe(
            fs.readFileSync('esercizio_js_45/output_test04.txt', 'utf-8').split('\n').toString()
        );
    });
    test('TEST 05', () => {
        expect(only_upper(
            fs.readFileSync('esercizio_js_45/input_test05.txt', 'utf-8')
        ).toString()).toBe(
            fs.readFileSync('esercizio_js_45/output_test05.txt', 'utf-8').split('\n').toString()
        );
    });
});
