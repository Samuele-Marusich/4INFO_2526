...
function aggiornaTotali(){
    ...
    for (let indice = 2; indice < tabella.length; indice++) {
        id = tabella.children[indice].classList[0]
        calcolaImportoRiga(id)        
    }
    if(tabellaTotalePresente == true){
        let somma = 0
        for (let indice = 2; indice < tabella.length; indice++) {
            somma = somma + Number(tabella.children[indice].children[3].innerText)            
        }
...
function mostraTotale(){
    ...
    if(tabellaCarrello.children.length >= 3){
        ...
        alert("Per utilizzare la funzione somma è necessario compilare tutti indice campi.")
    }
}
function elimina(elemento){
    const idPulsante = elemento.classList[0]
    console.log("caata")
