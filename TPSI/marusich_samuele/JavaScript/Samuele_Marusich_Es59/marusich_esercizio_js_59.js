...
describe("ESERCIZIO 59", () => {
  test("TEST 01", () => {
    expect(deduplicate([1, 3, 2, 1, 4, 3, 1])).toStrictEqual(
      Array.from(new Set([1, 3, 2, 4]))
    );
  });
  test("TEST 02", () => {
    expect(
      deduplicate([1, 3, 2, 1, 4, 3, 1, 5, 6, 10, 1010, 50, 50, 1, 32])
    ).toStrictEqual(Array.from(new Set([1, 3, 2, 4, 5, 6, 10, 1010, 50, 32])));
  });
  test("TEST 03", () => {
    expect(deduplicate(array)).toStrictEqual(Array.from(new Set(array)));
  });
  test("TEST 04", () => {
    expect(deduplicate(array2)).toStrictEqual(Array.from(new Set(array2)));
  });
  test("TEST 05", () => {
    expect(deduplicate(array3)).toStrictEqual(Array.from(new Set(array3)));
  });
});
